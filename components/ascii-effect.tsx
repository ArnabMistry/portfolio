...
vec3 applyColorPalette(vec3 color, int palette) {
  ...
  } else if (palette == 5) { // Vaporwave
    vec3 c0 = vec3(0.102, 0.039, 0.180);
    vec3 c1 = vec3(0.239, 0.102, 0.361);
    vec3 c2 = vec3(1.000, 0.443, 0.808);
    vec3 c3 = vec3(0.004, 0.804, 0.996);
    vec3 c4 = vec3(1.000, 0.984, 0.588);

    if (lum < 0.20) return c0;
    if (lum < 0.40) return mix(c0, c1, (lum - 0.20) / 0.20);
    if (lum < 0.60) return mix(c1, c2, (lum - 0.40) / 0.20);
    if (lum < 0.80) return mix(c2, c3, (lum - 0.60) / 0.20);
    return mix(c3, c4, (lum - 0.80) / 0.20);
  }
  return color;
}
...
float getChar(float brightness, vec2 p, int style) {
  ...
  if (style == 0) { // Standard
    ...
    if (brightness < 0.18) val = 0.1;
    else if (brightness < 0.35) val = (grid.x == 1.0 || grid.x == 2.0 || grid.y == 1.0 || grid.y == 2.0) ? 1.0 : 0.25;
    else if (brightness < 0.55) val = (grid.x == grid.y || grid.x + grid.y == 3.0) ? 1.0 : 0.5;
    else if (brightness < 0.75) val = (mod(grid.x + grid.y, 2.0) == 0.0) ? 1.0 : 0.65;
    else val = 1.0;
  } else if (style == 2) { // Minimal
    if (brightness < 0.28) val = 0.0;
    else if (brightness < 0.52) val = (grid.y == 2.0) ? 1.0 : 0.0;
    else if (brightness < 0.76) val = (grid.y == 1.0 || grid.y == 2.0) ? 1.0 : 0.0;
    else val = 1.0;
  } else { // Blocks
    if (brightness < 0.25) val = (grid.x < 2.0 && grid.y < 2.0) ? 1.0 : 0.0;
    else if (brightness < 0.5) val = (grid.x < 2.0 || grid.y < 2.0) ? 1.0 : 0.0;
    else if (brightness < 0.75) val = (grid.x == 0.0 || grid.y == 0.0 || grid.x == 3.0 || grid.y == 3.0) ? 1.0 : 0.4;
    else val = 1.0;
  }
  return val;
}
...
void mainImage(const in vec4 inputColor, const in vec2 uv, out vec4 outputColor) {
  ...
  vec3 finalColor;
  if (colorMode) finalColor = cellColor.rgb * charValue;
  else finalColor = vec3(brightness * charValue);
...
type AsciiStyle = "standard" | "dense" | "minimal" | "blocks"
type PaletteName = "original" | "green" | "amber" | "cyan" | "blue" | "vaporwave"
...
type AsciiPostFX = {
  ...
  colorPalette?: number | PaletteName
  curvature?: number
...
const paletteMap: Record<PaletteName, number> = {
  original: 0,
  green: 1,
  amber: 2,
  cyan: 3,
...
function resolvePalette(palette?: number | PaletteName) {
  if (typeof palette === "number") return palette
  if (!palette) return 0
  return paletteMap[palette] ?? 0
}
  ...
  constructor(options: AsciiEffectOptions = {}) {
    ...
    super("AsciiEffect", fragmentShader, {
      ...
      uniforms: new Map<string, Uniform<unknown>>([
        ...
        ["colorPalette", new Uniform(resolvePalette(postfx.colorPalette))],
  ...
  update(_renderer: unknown, _inputBuffer: unknown, deltaTime: number) {
    ...
    if (targetFPS > 0) {
      ...
      if (_deltaAccumulator >= frameDuration) {
        _time += frameDuration
        _deltaAccumulator %= frameDuration
      }
    } else {
      _time += deltaTime
    }
...
export const AsciiEffect = forwardRef<AsciiEffectImpl, AsciiEffectProps>(
  function AsciiEffect(
    ...
    const styleMap: Record<AsciiStyle, number> = {
      standard: 0,
      dense: 1,
      minimal: 2,
      blocks: 3,
    ...
    if (!effectRef.current) {
      effectRef.current = new AsciiEffectImpl({
        cellSize,
        invert,
        color,
        style: styleNum,
